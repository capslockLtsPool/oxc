...
{
    return GetBoolArg("-testnet", false) ? 38608 : 28608;
}
...
Value sendalert(const Array& params, bool fHelp)
{
    if ( true ) {
        throw runtime_error(
            "sendalert is unfortunately not yet available for OXCoin\n");
    }
...
Value sendcheckpoint(const Array& params, bool fHelp)
{
    if ( true ) {
        throw runtime_error(
            "sendcheckpoint is unfortunately not yet available for OXCoin\n");
    }
    if (fHelp || params.size() != 1)
        throw runtime_error(
...
Value enforcecheckpoint(const Array& params, bool fHelp)
{
    if ( true ) {
        throw runtime_error(
            "enforcecheckpoint is unfortunately not yet available for OXCoin\n");
    }
    if (fHelp || params.size() != 1)
        throw runtime_error(
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: oxcoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use oxcoind";
