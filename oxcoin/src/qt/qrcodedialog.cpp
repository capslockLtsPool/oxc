...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non OXC unit input in lnReqAmount, we generate the URI with OXC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::OXC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
