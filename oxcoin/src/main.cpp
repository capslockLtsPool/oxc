...
// Copyright (c) 2013 The OXCoin developers
...
uint256 hashGenesisBlock("00000d27423b38ebfb51af9520bf8e34b2a5bd2d4cfb3296cd26479e0ec5f30e");
unsigned int timeGenesisBlock = 1392901200;
...
const string strMessageMagic = "OXCoin Signed Message:\n";
 ...
 * individual tests with: ./test_oxcoin -t GetBlockValue_tests
 *
...
{
    int64 nSubsidy = 0;
    static int BlockValueTable[] = {25000, 15000, 7500, 3500, 3000, 2500, 2000, 1500, 1000};
    const int test_stage = 500; // 500 blocks for test
    const int stable_stage = 10080 * 50 + test_stage; // after 50 weeks, block reward is 30
    const int no_reward_stage = 10080 * 1300 + test_stage; // after 1300 weeks, block reward is 0
    
    if (nHeight == 0) {
        // Block 0, can't be spent
        nSubsidy = nGenesisBlockRewardCoin;
    } else if (nHeight < test_stage) { // first 500 blocks
    	nSubsidy = 100000 * COIN; // total 500*100,000 = 50,000,000
    } else if (nHeight < stable_stage) {
    	int crossover = sizeof(BlockValueTable)/sizeof(*BlockValueTable);
    	int NumOfWeeks = (nHeight - test_stage) / 10080;
    	if (NumOfWeeks < crossover) {
    		nSubsidy = BlockValueTable[NumOfWeeks] * COIN;
    	} else {
    		nSubsidy = BlockValueTable[crossover - 1] * COIN;
    		
    		for(int i=0;i< (NumOfWeeks - crossover + 1);i++){
			nSubsidy=nSubsidy*92;
			nSubsidy=nSubsidy/100;
		}
    	}
    } else if (nHeight < no_reward_stage) {
    	nSubsidy = 30 * COIN;
    } else {
    	nSubsidy = 0;
    }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OXCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OXCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("OXCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("oxcoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running OXCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
